...
import { FormInput, MainHeader, Card11, MaterialIcon, Loading } from '../../../../components/react-native-teso';
...
import { bookingActions, profileActions, messengerActions, storeDetailActions } from '../../../../actions';
import { nameOfBookingReducers, nameOfProfileReducers, nameOfLoadingReducers, nameOfStoreDetailReducers } from '../../../../reducers';
...
class CreateBookingScreen extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            phone: '',
            gender: false,
            status: 'CONFIRMED', //Nhân viên đã nhận đơn hàng
    ...
    componentDidMount = () => {
        ...
        this.setState({
            ...
            storeId: get(storeService.getSpecificState(nameOfStoreDetailReducers), 'storeId')//[...get(storeService.getSpecificState(nameOfProfileReducers), 'storeList')][0].id,
        }, () => {
        ...
        const data = {
            ...
            callback: () => {
                this.setState({
                    ...
                    gender: this.props.detail.gender == 'Nam' ? true : false,
    ...
    addBooking = (address, firstName, lastName, phone, gender, status, bookDate, bookTime, guestCount, note, storeId) => {
        const booking = {
            'address': address,
            'userId': 0,
            'cusName': `${firstName} ${lastName || ''}`,
            ...
            'gender': gender === 0 ? 'Nam' : gender === 1 ? 'Nữ' : 'Khác',
            'status': status,
            'bookDate': bookDate,
            'bookTime': bookTime,
            'guestCount': guestCount,
        ...
        const data = {
            'booking': booking,
            callback: () => {
                storeService.dispatch(bookingActions.getEmployeesBookingRequest({
                    'booking': {
                        empIds: [get(storeService.getSpecificState(nameOfProfileReducers).user, 'id')]
                    }
                }));
                get(storeService.getSpecificState(nameOfStoreDetailReducers), 'data.ownerId') && storeService.dispatch(messengerActions.pushNotificationRequest({
                    "data": {
                        "title": `${booking.empName}`,
                        "content": `Lịch hẹn được chỉ định tiếp nhận cho đơn hàng mới được tạo bởi nhân viên ${booking.empName}`,
